...
Feature: CreditManagement - GetCreditProfile
  ...
  Background:
   *print Get Credit Profile by ID(CID)number for customer having assessmentMessageCd: TMSG067
 ...
 @GetCreditProfile_Positive_TC @GetCreditProfile @Regression_Suite @Regression_Suite_CreditProfile_TCs
  ...
  #Calling of Assessent messageCode Operation
 
    * def RandomEventID =  RandomNum.getRandom8digitNumber()
    * testData.eventId = RandomEventID
    * print testData
    
    * def response =  call read(PATH_SERVICES_CREATECPROFILE+'AssessmentMessageCode_Operation.feature') testData
    Then print "Final Create CP response: " + response.response

   
     ## #Asssertion on REST response
    And def resp = $.response
    Then match $.response.x-correlation-id != null 
    * print "x-correlation-id : " + $.response.x-correlation-id
 ...
 #Calling of Get Credit Profile Operation
    ...
    * print "Validate Get Response value"
    Then match $.response.lastRiskAssessment.assessmentMessageCd == 'KMSG002'
    Then match $.response.lastRiskAssessment.assessmentMessageTxtEn == <assessmentMessage>
    Then match $.response.lastRiskAssessment.assessmentMessageTxtFr != null
    ...
    Then match $.response.clpRatePlanAmt == 170.0
...
#Assertion data in READDB_SYNC_STATUS TABLE
    ...
    #Then match ReadDBSynResult[0].need_to_sync == false
       ...
       Examples:
      | testScenario                  | Id   |  ReffTyp          |creditRiskLevelNum| creditSCR|creditRiskLevelDecisionCd|creditDecisionCd| bureauDC | creditCls | creditPrgNM | WarngCd |WarngCd2|province| warStsCd   | warCd  |warCd2|role1   |warningDetectionTs      |assessmentMessage|
      | TC9_getCP_AssessmentMessageCd | 6000 |  'RiskAssessment' | 600              |      600 |'RL001'                  |'D001'          | 'E29'    | 'L'       | 'CLP'       | 'BLF'   |   'BCW'|'BC'    | 'VERIFIED' | 'F'    | 'F'  |'Customer'|'2021-12-15T20:49:01.839Z'|'Decline Service'|
