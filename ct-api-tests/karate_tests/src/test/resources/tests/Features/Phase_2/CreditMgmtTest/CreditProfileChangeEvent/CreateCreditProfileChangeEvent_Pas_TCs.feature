#Phase2
Feature: CreditManagement - CreateCreditProfile_ChangeEvent

  Background: 
    *print  CreateCreditProfile_ChangeEvent HappyPath test


   @CreateCreditProfileChange_event_Positive_TC  @CreateCreditProfileChange_event @Regression_Suite @Regression_Suite_CreditProfile_TCs
  Scenario Outline: CreateCreditProfile_ChangeEvent : Valid Data with <testScenario>
    ...
    * print  'RandomCustomerID=='+ RandomCustomerID
     ...
     * def RandomEventID =  RandomNum.getRandom8digitNumber()
    * testData.eventId = RandomEventID
      * print  'RandomEventID=='+ RandomEventID
    
    * def RandomcreditProfileLegacyId =  RandomNum.getRandom6digitNumber()
    * testData.creditProfileLegacyId = RandomcreditProfileLegacyId
     * print  'RandomcreditProfileLegacyId=='+ RandomcreditProfileLegacyId
    
     * def RandomwarningHistoryLegacyId =  RandomNum.getRandom4digitNumber()
    * testData.warningHistoryLegacyId = RandomwarningHistoryLegacyId
     * print  'RandomwarningHistoryLegacyId=='+ RandomwarningHistoryLegacyId
    
    * def RandomwarningHistoryLegacyIdNew =  RandomNum.getRandom4digitNumber()
    * testData.warningHistoryLegacyIdNew = RandomwarningHistoryLegacyIdNew
     * print  'RandomwarningHistoryLegacyIdNew=='+ RandomwarningHistoryLegacyIdNew
    
    
    
    * testData.Id=RandomCustomerID
    * set testData.payloadName = 'CreditProfile_ChangeEvent/CreditProfile_ChangeEvent_payload.json'
    * print testData
    * def response =  call read(PATH_SERVICES+'CreditProfile_ChangeEvent/CreditProfile_ChangeEvent_Operation.feature') testData
    ...
    * print "CustomerID : " + CustomerID
    And match CustomerID == RandomCustomerID
     ...
     And def creditProfileLegacyId = $.response.creditProfileLegacyId
    * print "creditProfileLegacyId : " + creditProfileLegacyId
    And match creditProfileLegacyId == RandomcreditProfileLegacyId
    ...
    * def Decryptcredit_class_code = new CryptoServices().decrypt(crdProfileResult[0].credit_class_code)
    ...
    Then match crdProfileResult[0].credit_profile_id_legacy == RandomcreditProfileLegacyId
    ...
    * def CrdWarQry = testSpecFileRead.Queries.CrdWarningLegacyIdQry
    ...
    * string DBwarningHistoryLegacyId =  CrdWarResult[0].warning_legacy_id
    Then match DBwarningHistoryLegacyId  ==  testData.warningHistoryLegacyId
    ...
    * def Decryptwarning_Item_Type_Cd = new CryptoServices().decrypt(CrdWarResult[0].warning_Item_Type_Cd)
    ...
    Examples: 
      | testScenario                                                                                                        | ReffTyp          | creditRiskLevelNum | creditSCR | creditRiskLevelDecisionCd | creditDecisionCd | bureauDC | creditCls | creditPrgNM | WarngCd | WarngCd2 | province | warStsCd   | warCd | warCd2 | role1    | warningDetectionTs       |version|
      | Validate response received/Write DB details after hitting create credit profile Change event API with event Payload. | 'RiskAssessment' |               2019 |       500 | 'RL001'                   | 'D001'             | 'E29'    | 'B'       | 'CLP'       | 'BCW'   | 'BCW'    | 'AB'   | 'UNVERIFIED' | 'M'   | 'F'  |'Customer'| '2021-12-15T20:49:01.839Z' |2|
