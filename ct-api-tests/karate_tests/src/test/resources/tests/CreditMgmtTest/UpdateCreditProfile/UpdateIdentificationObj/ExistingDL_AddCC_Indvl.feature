...
Feature: CreditManagement - CreateCreditProfile
  ...
  Scenario Outline: UpdateCreditProfile : <testScenario>
    ...
    * set testData.payloadUpdate = 'IndvlIdentification_payload.json'
    * def response =  call read(PATH_SERVICES_UPDATECPROFILE+'UpdateCreditProfile_Operation.feature') testData
    ...
    And match Id == Id
    #Response Validations
    Then match res.engagedParty.individualIdentification[1].identificationId == testData.IdNumUpdate
    Then match res.engagedParty.individualIdentification[1].validFor.endDateTime == testData.endDateTimeUpdate
    Then match res.engagedParty.individualIdentification[1].validFor.startDateTime == testData.startDateTimeUpdate
    ...
    * print 'IDENTIFICATION_CHAR_HASH TABLE Result: ' + IdentCharHashResult
    ...
    Examples: 
      | testScenario         | data               |
      | 2_Existing DL Add CC | 'ExistingDL_AddCC' |
