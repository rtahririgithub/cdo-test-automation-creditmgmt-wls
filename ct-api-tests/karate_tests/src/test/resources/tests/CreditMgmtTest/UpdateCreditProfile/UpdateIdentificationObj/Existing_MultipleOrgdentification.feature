@HappyPath @OrgHCDlIdentification
Feature: CreditManagement - CreateCreditProfile
  ...
  Scenario Outline: UpdateCreditProfile : <testScenario>
    ...
    * set testData.Id = 2 + generateRandomNumber(3)
    * set testData.payloadName = 'MultipleOrgIdentificationTyp_payload.json'
    * print testData
    ...
    * print 'IDENTIFICATION_CHAR_HASH TABLE Result: ' + IdentCharHashResult
    ...
    Examples: 
      | testScenario                                          | data               |
      | 5_Update Existing HC where customer already has DL&HC | 'Existing_HCAndDL' |
