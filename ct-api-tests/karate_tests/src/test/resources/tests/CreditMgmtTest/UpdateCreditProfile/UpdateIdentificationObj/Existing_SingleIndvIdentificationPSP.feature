@HappyPath @PSPIdentificationPSP
Feature: CreditManagement - CreateCreditProfile
  ...
  Scenario Outline: UpdateCreditProfile : <testScenario>
    ...
    * set testData.payloadUpdate = 'IndvlIdentificationPSP_payload.json'
    * def response =  call read(PATH_SERVICES_UPDATECPROFILE+'UpdateCreditProfile_Operation.feature') testData
    ...
    And match Id == Id
    #Response Validations
    Then match res.engagedParty.individualIdentification[0].identificationId == testData.IdNumUpdate
    Then match res.engagedParty.individualIdentification[0].telusCharacteristic.countryCd == testData.countryUpdate
    Then match res.engagedParty.individualIdentification[0].validFor.endDateTime == testData.endDateTimeUpdate
    Then match res.engagedParty.individualIdentification[0].validFor.startDateTime == testData.startDateTimeUpdate
    ...
    * def IdntID_id = PartyIdentificationResult[0].identificaton_id
    Then match PartyIdentificationResult[0].id_type == testData.id_typ
    ...
    * print 'IDENTIFICATION_CHAR TABLE Result: ' + IdentificationCharResult
    ...
    Then match IdentificationCharResult[0] contains { value: '#(testData.countryUpdate)' }
    Then match IdentificationCharResult[1] contains { value: '#(testData.countryUpdate)' }
    Then match IdentificationCharResult[0].version == 2
    Then match IdentificationCharResult[1].version == 2
    Then match IdentificationCharResult[2].version == 2
    ...
    * print 'IDENTIFICATION_CHAR_HASH TABLE Result: ' + IdentCharHashResult
    Then match IdentCharHashResult[0] contains { Key:'identificationId' }
     Then match IdentCharHashResult[0].version == 2

    Examples: 
      | testScenario                             | data           |
      | Update existing PSP with new PSP details | 'Existing_PSP' |
