...
#######################33
    ...
    * print "Validate Get Response value"
    ...
    Then match $.response.engagedParty.organizationIdentification[0].identificationId == IdNum
    Then match $.response.engagedParty.organizationIdentification[0].identificationType == <idTyp>
    Then match $.response.engagedParty.organizationIdentification[0].issuingAuthority == <province>
    #Then match $.response.engagedParty.organizationIdentification[0].telusCharacteristic.provinceCd == <province>
    #Then match $.response.engagedParty.organizationIdentification[0].telusCharacteristic.identificationTypeCd == <id_typ>
    ...
    * print 'READDB_SYNC_STATUS TABLE Result: ' + ReadDBSynResult
    ...
    Examples:
      | testScenario                     | Id   | idTyp                          | DOB          | ReffTyp        | creditSCR | bureauDC | creditCls | creditPrgNM | WarngCd | Country | province | medTyp          | IdNum     | id_typ | warStsCd   | warCd   | crdtCls | crdtSCR | CPrgNM | rRtng | city
      | TC3_getCPOrganizational_IdTypBIC | 3042 | 'Business Registration Number' | '1990-02-01' | 'Organization' |       500 | 'E29'    | 'B'       | 'NDP'       | 'CCW'   | 'CAN'   | 'QC'     | 'postalAddress' | 843785112 | 'BIC'  | UNVERIFIED | CITADEL | 'L' | 470 | 'NDP' | 400        | 'Vancouver'
